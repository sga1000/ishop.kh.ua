...
final class Loader {
	...
	public function model($model, $data = array()) {
		...
		if (file_exists($file)) {
			if($this->registry->get('model_' . str_replace('/', '_', $model)) === null){ // Проверяем существует ли модель уже, т.е. загружена ли ранее
				include_once(modification($file));
				if(method_exists($class,'getInstance')){
					// уТт получаем инстанс
					$this->registry->set('model_' . str_replace('/', '_', $model), $class::getInstance($this->registry));
				} else {
					//тут делаем через нью
					$this->registry->set('model_' . str_replace('/', '_', $model), new $class($this->registry));
				}
			} else {
				// МОдель уде существует, ничделаем ничего.
			}
		} else {
