...
class MainSmsRu
{
	...
	protected function log($message)
	{
		if (!$this->debug)
			return;
		file_put_contents(DIR_LOGS . $this->_logFile, date("Y-m-d H:i:s - ") . "mainsms.ru: " . $message . "\r\n", FILE_APPEND);
	}
	...
	public function send()
	{
		...
		$this->messageSend($this->phone, $this->message, $this->sender, null);
		if (!$this->response) {
			$this->log("Не удалось подключиться к шлюзу mainsms");
		} else {
			$this->log("ответ сервера: " . print_r($this->response, true));
		}
