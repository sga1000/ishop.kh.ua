...
$(document).delegate('input[name=\'city\']', 'keydown.autocomplete', function () {
	...
	$(this).autocomplete_address({
		'source': function (request, response) {
			$.ajax({
				url: 'index.php?route=shipping/neoseo_novaposhta/cityautocomplete&filter_name=' + encodeURIComponent(request),
				dataType: 'json',
		...
		'select': function (item) {
			$(this).val(item['label']);
			$.ajax({
				url: 'index.php?route=shipping/neoseo_novaposhta/savecity&city_id=' + item['value']
			});
		}
	});
});
...
{
	$.ajax({
		url: 'index.php?route=shipping/neoseo_novaposhta/savewarehouse&wh_id=' + $('#neoseo_novaposhta_selected_warehouse').val()
	});
}
...
{
	$.ajax({
		url: 'index.php?route=shipping/neoseo_novaposhta/saveCourierAdress&address=' + $('#neoseo_novaposhta_address').val()
	});
}
...
$('#simplecheckout_button_confirm').on('click', np_validator());