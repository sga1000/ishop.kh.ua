...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		$order_id = $this->db->getLastId();
                ...
                $first = false; // Возможно кука уже была удалена
                if (isset($this->request->cookie['first_referrer'])) {
                    $referrer_id = $this->request->cookie['first_referrer'];
                    $this->db->query("INSERT INTO `" . DB_PREFIX . "order_referrer` (order_id, time_added, cookie_name, referrer ) SELECT '" . (int)$order_id . "' as order_id, time_added, cookie_name, referrer FROM `" . DB_PREFIX . "referrer` WHERE `referrer_id`='" . (int)$referrer_id . "';");
                ...
                if (isset($this->request->cookie['last_referrer'])) {
                    $referrer_id = $this->request->cookie['last_referrer'];
                    if( !$first ) {
                        $this->db->query("INSERT INTO `" . DB_PREFIX . "order_referrer` (order_id, time_added, cookie_name, referrer ) SELECT '" . (int)$order_id . "' as order_id, time_added, 'first_referrer' as cookie_name, referrer FROM `" . DB_PREFIX . "referrer` WHERE `referrer_id`='" . (int)$referrer_id . "';");
                    }
                    $this->db->query("INSERT INTO `" . DB_PREFIX . "order_referrer` (order_id, time_added, cookie_name, referrer ) SELECT '" . (int)$order_id . "' as order_id, time_added, cookie_name, referrer FROM `" . DB_PREFIX . "referrer` WHERE `referrer_id`='" . (int)$referrer_id . "';");
                }
	...
	public function editOrder($order_id, $data) {
		...
		/* NeoSeo Email Notify - begin */
		$this->config->set("neoseo_email_notify_status",0); // Disables stupid notification
		/* NeoSeo Email Notify - end */
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
                ...
                /* NeoSeo Order Referrer - begin */
                $referrer_query = $this->db->query("SELECT `referrer` FROM `" . DB_PREFIX . "order_referrer` WHERE order_id = '" . (int)$order_id . "' and cookie_name='first_referrer'");
                if ($referrer_query->num_rows) {
                    $first_referrer = $referrer_query->row['referrer'];
                } else {
                    $first_referrer = '';
                }

                $referrer_query = $this->db->query("SELECT `referrer` FROM `" . DB_PREFIX . "order_referrer` WHERE order_id = '" . (int)$order_id . "' and cookie_name='last_referrer'");
                if ($referrer_query->num_rows) {
                    $last_referrer = $referrer_query->row['referrer'];
                } else {
                    $last_referrer = '';
                }
                /* NeoSeo Order Referrer - end */
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		/* NeoSeo Email Notify - begin */
        $this->load->controller('module/neoseo_email_notify/handler_order_history_add',array(
            'order_id' => $order_id,
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$mail->setHtml($html);
				$mail->setText($text);
				/* NeoSeo Email Notify - begin */ /* $mail->send(  ); */ /* NeoSeo Email Notify - end */
                ...
                /* NeoSeo Order Referrer - begin */
                $text .= $language->get('text_first_referrer') . " " . $order_info["first_referrer"] . "\r\n";
                $text .= $language->get('text_last_referrer') . " " . $order_info["last_referrer"] . "\r\n";
                /* NeoSeo Order Referrer - end */
