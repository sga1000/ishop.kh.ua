...
class ControllerExtensionFeedFacebookLite extends Controller {
	private $currencies = array();
	private $categories = array();
	private $eol = ""; 

	public function index() {
		if ($this->config->get('remarketing_facebook_feed_status')) {
		
		$this->load->model('extension/feed/facebook_lite');
		...
		$offers_currency = $this->config->get('remarketing_facebook_currency');
		...
		$products = $this->model_extension_feed_facebook_lite->getProducts();
		
		foreach ($products as $product) { 
			$output .= '<item>'. $this->eol; 
			$output .= '<g:id>' . ($this->config->get('remarketing_facebook_id') == 'id' ? $product['product_id'] : $product['model']) . '</g:id>' . $this->eol;
			...
			$output .= '<g:image_link>' . $this->model_tool_image->resize($product['image'], $this->config->get($this->config->get('config_theme') . '_image_popup_width'), $this->config->get($this->config->get('config_theme') . '_image_popup_height')) . '</g:image_link>' . $this->eol;
