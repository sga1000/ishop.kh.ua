 ...
 */
class ControllerExtensionFeedYandexLite extends Controller {
	private $currencies = array();
	private $categories = array();
	private $eol = ""; 

	public function index() {
		if ($this->config->get('remarketing_yandex_feed_status')) {
		
		$this->load->model('extension/feed/yandex_lite');
		...
		$offers_currency = $this->config->get('remarketing_yml_currency');
		...
		$categories = $this->model_extension_feed_yandex_lite->getCategories();
		...
		$products = $this->model_extension_feed_yandex_lite->getProducts();
		
		foreach ($products as $product) {
			$output .= '<offer id="' . ($this->config->get('remarketing_mytarget_id') == 'id' ? $product['product_id'] : $product['model']) . '" available="' . ($product['quantity'] > 0 ? 'true' : 'false') . '">' . $this->eol;
			...
			$output .= '<picture>' . $this->model_tool_image->resize($product['image'], $this->config->get($this->config->get('config_theme') . '_image_popup_width'), $this->config->get($this->config->get('config_theme') . '_image_popup_height')) . '</picture>' . $this->eol;
