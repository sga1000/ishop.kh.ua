...
class ControllerModuleNeoseoFeatured extends NeoSeoController
{
	...
	public function index($setting)
	{
		...
		$limit = (int)$this->config->get('neoseo_unistor_module_preview_count');
		...
		$data['use_banner'] = isset($setting['use_banner']) ? $setting['use_banner'] : false;
		$data['banner'] = isset($setting['banner']) ? $setting['banner'] : '';
		$data['banner_width'] = (isset($setting['banner_width']) && (int)$setting['banner_width'] > 0) ? $setting['banner_width'] : 250;
		$data['banner_height'] = (isset($setting['banner_height']) && (int)$setting['banner_height'] > 0) ? $setting['banner_height'] : 250;
		$this->load->model('tool/image');
		if($data['use_banner']&&$data['banner']){
			$data['image_banner'] = $this->model_tool_image->resize($data['banner'], $data['banner_width'], $data['banner_height']);
			$data['banner_link'] = $setting['banner_link'][$this->config->get('config_language_id')];
			$data['block_description'] = html_entity_decode($setting['block_description'][$this->config->get('config_language_id')]);
		}else{
			$data['image_banner']='';
			$data['banner_link']='';
			$data['block_description']='';
		}
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/neoseo_featured_' . $template . '.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/module/neoseo_featured_' . $template . '.tpl', $data);
		} else {
			return $this->load->view('default/template/module/neoseo_featured_' . $template . '.tpl', $data);
		}
	}
