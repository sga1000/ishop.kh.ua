...
class ControllerModuleNeoseoLatest extends NeoSeoController
{
	...
	public function index($setting)
	{
		...
		$data['use_image'] = isset($setting['use_image'])?$setting['use_image']:false;
		$data['image_width'] = (isset($setting['image_width'])&&$setting['image_width']>0)?$setting['image_width']:400;
		$data['image_height'] = (isset($setting['image_height'])&&$setting['image_height']>0)?$setting['image_height']:400;

		if (isset($setting['image']) && is_file(DIR_IMAGE . $setting['image'])) {
			$data['block_image'] = $this->model_tool_image->resize($setting['image'], $data['image_width'],  $data['image_height']);
		} else {
			$data['block_image'] = $this->model_tool_image->resize('no_image.png', $data['image_width'],  $data['image_height']);
		}
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/neoseo_latest_' . $template . '.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/module/neoseo_latest_' . $template . '.tpl', $data);
		} else {
			return $this->load->view('default/template/module/neoseo_latest_' . $template . '.tpl', $data);
		}
	}
