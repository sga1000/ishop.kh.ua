	...
	<?php if($send_request) { ?>	
		<div class="tile-body module-absent">
			...
			<div id="range" class="dropdown-menu dropdown-menu-right dropdown-absent">
				<form id="request_callback_widget">
					...
					<div class="form-group">
						<div class="col-sm-12">
							<button class="btn-tile btn-absent-request" data-original-title="<?php echo $button_request; ?>" onclick='sendRequestCallbackWidget();
...
<script type="text/javascript">
	function sendRequestCallbackWidget() {
		var domain = $("#request_callback_widget input[name=\'domain\']").val();
		var email = $("#request_callback_widget input[name=\'email\']").val();

		$.ajax({
			url: 'index.php?route=dashboard/neoseo_callback_widget/sendRequest&token=<?php echo $token; ?>&domain=' + encodeURIComponent(domain) + '&email=' + encodeURIComponent(email),
			dataType: 'json',
			success: function (json) {
				$('.alert, .text-danger, .alert, .text-success').remove();

				if (json.error) {
					$('#request_callback_widget').before('<div class="alert alert-danger"><i class="fa fa-check-circle"></i> ' + json.result + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
				} else {
					$('#request_callback_widget').before('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + json.result + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
				}
			},
