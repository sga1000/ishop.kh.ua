			...
			<div class="panel-body">
				...
				<ul class="nav nav-tabs">
					...
					<?php if( !isset($license_error) ) { ?>						
						...
						<li><a href="#tab-notification_admin" data-toggle="tab"><?php echo $tab_notification_admin; ?></a></li>
					...
					<div class="tab-content">
						...
						<?php if( !isset($license_error) ) { ?>
							 <div class="tab-pane" id="tab-notification">
								<?php $widgets->localeTextarea('reg_text', $full_languages); ?>
								 <?php  if ($widgets->neoseo_approve_email_theme_mails_text==2) { ?>
              <div class="text-danger"><?php echo $error_neoseo_approve_email; ?></div>
              <?php } ?>
								<?php $widgets->localeInput('theme_mails_text', $full_languages); ?>
								<?php $widgets->localeTextarea('mails_text', $full_languages); ?>							
								<?php $widgets->localeTextarea('reg_true_text', $full_languages); ?>
							</div>
							
							<div class="tab-pane" id="tab-notification_admin">
								<?php $widgets->input('mails_admin'); ?>
								<?php $widgets->localeInput('theme_mails_text_admin', $full_languages); ?>
								<?php $widgets->localeTextarea('mails_text_admin', $full_languages); ?>
...
<?php if ($ckeditor) { ?>
		ckeditorInit('neoseo_approve_email_reg_text<?php echo $language['language_id']; ?>', '<?php echo $token; ?>');
		ckeditorInit('neoseo_approve_email_mails_text<?php echo $language['language_id']; ?>', '<?php echo $token; ?>');
		ckeditorInit('neoseo_approve_email_reg_true_text<?php echo $language['language_id']; ?>', '<?php echo $token; ?>');
		ckeditorInit('neoseo_approve_email_mails_text_admin<?php echo $language['language_id']; ?>', '<?php echo $token; ?>');
<?php } else { ?>
		$('#neoseo_approve_email_reg_text<?php echo $language['language_id']; ?>').summernote({height: 300, lang:'<?php echo $lang; ?>'});
		$('#neoseo_approve_email_mails_text<?php echo $language['language_id']; ?>').summernote({height: 300, lang:'<?php echo $lang; ?>'});
		$('#neoseo_approve_email_reg_true_text<?php echo $language['language_id']; ?>').summernote({height: 300, lang:'<?php echo $lang; ?>'});
		$('#neoseo_approve_email_mails_text_admin<?php echo $language['language_id']; ?>').summernote({height: 300, lang:'<?php echo $lang; ?>'});
<?php } ?>
<?php } ?>
//--></script>
<?php echo $footer; ?>