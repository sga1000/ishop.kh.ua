...
$widgets = new NeoseoWidgets('neoseo_featured_',$params);
          ...
          <div class="tab-content">
            <div class="tab-pane active" id="tab-general">
              ...
              <div class="form-group">
                <label class="col-sm-5 control-label" for="input-product"><?php echo $entry_product; ?></label>
                <div class="col-sm-7">
                  ...
                  <div id="featured-product" class="well well-sm" style="height: 150px; overflow: auto;">
                    ...
                    <div id="featured-product<?php echo $product['product_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $product['name']; ?>
                      <input type="hidden" name="neoseo_featured_products[]" value="<?php echo $product['product_id']; ?>" />
                    </div>
                    <?php } ?>
                  </div>
                </div>
              ...
              <?php $widgets->localeTextarea('block_description',$full_languages); ?>
              <?php /* // Убираем пока банер
              <?php $widgets->dropdown('use_banner',array( 0 => $text_disabled, 1 => $text_enabled)); ?>
              <div class="form-group">
                <label class="col-sm-5 control-label" for="input-product"><?php echo $entry_banner; ?></label>
                <div class="col-sm-7">
                  <a href="" id="banner-image" data-toggle="image" class="img-thumbnail" ><img style="max-width:100px; max-height: 100px;" src="<?php echo $image_banner ?>" alt="" title="" data-placeholder="<?php echo $image_banner; ?>" /></a>
                  <input type="hidden" name="<?php echo $sysname; ?>_banner" value="<?php echo ${$sysname.'_banner'} ?>" id="input-image" />
                </div>
              </div>
              <?php $widgets->input('banner_width'); ?>
              <?php $widgets->input('banner_height'); ?>
              <?php $widgets->localeInput('banner_link',$full_languages); ?>
              */ ?>
              <?php } else { ?>
...
<script type="text/javascript"><!--
	$('input[name=\'product\']').autocomplete({
		...
		select: function(item) {
			...
			$('#featured-product').append('<div id="featured-product' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="neoseo_featured_products[]" value="' + item['value'] + '" /></div>');
		}
	});
	...
	<?php if ($ckeditor) { ?>
			ckeditorInit('neoseo_featured_block_description<?php echo $language['language_id']; ?>', '<?php echo $token; ?>');
		<?php } else { ?>
			$("#neoseo_featured_block_description<?php echo $language['language_id']?>").summernote({height: 300, lang:'ru-RU'});
		<?php } ?>
	<?php } ?>
    //--></script>
<?php echo $footer; ?>