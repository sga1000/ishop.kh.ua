...
class ControllerAnalyticsProgressiveWebApplication extends Controller {
	...
	public function index() {
		...
		$labels = $this->load->language('analytics/progressive_web_application');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (empty($this->request->post['progressive_web_application_status']['status']) || ($this->request->post['progressive_web_application_status']['status'] == '0')) {
				...
				$this->model_setting_setting->editSettingValue('progressive_web_application', 'progressive_web_application_status', 0, $store_id);

			} else {

				$this->model_setting_setting->editSetting('progressive_web_application', $this->request->post, $store_id);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('analytics/progressive_web_application', 'token='.$this->session->data['token'].'&store_id='.$store_id, 'SSL')
		);

		$data['action'] = $this->url->link('analytics/progressive_web_application', 'token='.$this->session->data['token'].'&store_id='.$store_id, 'SSL');

		$data['cancel'] = $this->url->link('extension/analytics', 'token='.$this->session->data['token'], 'SSL');

		$data['send_ticket'] = str_replace('&amp;', '&', $this->url->link('analytics/progressive_web_application/send_ticket', 'token='.$this->session->data['token'].'&store_id='.$store_id, 'SSL'));
		...
		$data['stores'][] = array(
			...
			'url'      => $this->url->link('analytics/progressive_web_application', 'token='.$this->session->data['token'].'&store_id=0', 'SSL')
		);
		...
		if ($results) {
			...
			foreach ($results as $key => $value) {
				...
				$data['stores'][] = array(
					...
					'name'     => $value['name'],
					'url'      => $this->url->link('analytics/progressive_web_application', 'token='.$this->session->data['token'].'&store_id='.$value['store_id'], 'SSL')
				);
			}
		...
		$data['settings'] = array();
		$data['settings'] = json_decode($this->model_setting_setting->getSettingValue('progressive_web_application_status', $store_id), true);
		...
		$this->response->setOutput($this->load->view('analytics/progressive_web_application.tpl', $data));
	...
	public function send_ticket() {
		...
		$this->load->language('analytics/progressive_web_application');

		if (!$this->user->hasPermission('modify', 'analytics/progressive_web_application')) {
