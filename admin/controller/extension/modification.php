...
class ControllerExtensionModification extends Controller {
	...
	public function refresh() {
		...
		if ($this->validate()) {
			...
			$template_prefixes = $this->model_tool_neoseo_unistor->getTemplatePrefixes();
			unset($template_prefixes['default']);
			//Log
			$log = array();
					...
					foreach ($files as $file) {
						...
						if ($path) {
							...
							foreach ($files as $file){
								...
								foreach ($template_prefixes as $template_prefix){
									$dummy_filename = $path_parts['dirname'].'/'.$path_parts['filename']."[{$template_prefix}]".$path_parts['extension'];
									if(is_file($dummy_filename) && !in_array($dummy_filename,$files)){
										$files[] = $dummy_filename;
									}
								}
							}
							//echo "<pre>". print_r($files,true);exit;
							if ($files) {
								foreach ($files as $file) {
