...
class ControllerCustomerCustomer extends Controller {
		...
		$data['order'] = $order;
                ...
                if( isset($this->session->data['token']) ) {
                    if( $this->user->hasPermission('access','customer/neoseo_subscribe') && $this->config->get("neoseo_subscribe_status")){
                        ...
                        $data['button_export_neoseo_subscribe'] = $this->language->get("button_export");
                        $data['action_export_neoseo_subscribe'] = $this->url->link('customer/neoseo_subscribe/exportCustomerData', 'token=' .$this->session->data['token'], 'SSL');
                    }
                }
	...
	protected function getForm() {
		/* NeoSeo Customer Order History - begin */
		if (isset($this->request->get['customer_id'])) {
			$customer_id = $this->request->get['customer_id'];
		}else{
			$customer_id = 0;
		}
        $this->language->load('customer/neoseo_customer_order_history');
		$this->load->model('customer/neoseo_customer_order_history');
		$filter = array(
			'customer_id' => $customer_id,
		);
		$data['tab_orders'] = $this->language->get('tab_orders');
		$data['orders'] = $this->model_customer_neoseo_customer_order_history->getOrders($filter);
		/* NeoSeo Customer Order History - end */
