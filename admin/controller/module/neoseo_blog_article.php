...
class ControllerModuleNeoSeoBlogArticle extends NeoSeoController
{
	private $error = array();
	...
	public function index()
	{
		...
		$data = $this->load->language('module/neoseo_blog_article');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('neoseo_blog_article', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('module/neoseo_blog_article', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title_raw'),
				'href' => $this->url->link('module/neoseo_blog_article', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('module/neoseo_blog_article', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('module/neoseo_blog_article', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		$files = glob(DIR_CATALOG . 'view/theme/default/template/module/neoseo_blog_article*.tpl');
		if ($files) {
		...
		$this->response->setOutput($this->load->view('module/neoseo_blog_article.tpl', $data));
	}

	protected function validate()
	{
		if (!$this->user->hasPermission('modify', 'module/neoseo_blog_article')) {
