...
class ControllerModuleNeoSeoAuthSms extends NeoSeoController
{
	...
	public function index()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			...
			if (!$this->config->get('neoseo_sms_notify_status') == 1) {
				$this->request->post[$this->_moduleSysName() . '_status'] = '0';
				$this->session->data['error_warning'] = $this->language->get('error_sms_notify_status');
			}
		...
		$data['captchas'] = array();
		$data['captchas'][0] = $this->language->get('text_select');
		...
		foreach ($extensions as $code) {
			...
			if ($this->config->has($code . '_status')) {
				$data['captchas'][$code] = $this->language->get('heading_title');
			}
		}
