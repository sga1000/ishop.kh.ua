...
class ControllerModuleNeoSeoLatest extends NeoSeoController
{
	...
	public function __construct($registry)
	{
		...
		$this->_moduleSysName = "neoseo_latest";
	...
	public function index()
	{
		...
		$data = $this->initModuleParams(array(
			...
			array($this->_moduleSysName . '_use_image', 0),
			array($this->_moduleSysName . '_image', 'no_image.png'),
			array($this->_moduleSysName . '_image_width', 400),
			array($this->_moduleSysName . '_image_height', 400),
		...
		if(!($data[$this->_moduleSysName() . '_image_width'] > 0)) $data[$this->_moduleSysName() . '_image_width'] = 400;
		if(!($data[$this->_moduleSysName() . '_image_height'] > 0)) $data[$this->_moduleSysName() . '_image_height'] = 400;

		$data['image_placeholder'] = $this->model_tool_image->resize('no_image.png', $data[$this->_moduleSysName() . '_image_width'],  $data[$this->_moduleSysName() . '_image_height']);
		if ($data[$this->_moduleSysName() . '_image'] && is_file(DIR_IMAGE . $data[$this->_moduleSysName() . '_image'])) {
			$data['block_image'] = $this->model_tool_image->resize($data[$this->_moduleSysName() . '_image'], $data[$this->_moduleSysName() . '_image_width'],  $data[$this->_moduleSysName() . '_image_height']);
		} else {
			$data['block_image'] = $data['image_placeholder'];
		}
	...
	protected function validate()
	{
		...
		if(isset($this->request->post[$this->_moduleSysName . '_image_width']) && $this->request->post[$this->_moduleSysName . '_image_width'] == ''){
			$this->error['warning']= $this->language->get('error_width');
		}
		if(isset($this->request->post[$this->_moduleSysName . '_image_height']) && $this->request->post[$this->_moduleSysName . '_image_height'] == ''){
			$this->error['warning']= $this->language->get('error_height');
		}
		if (isset($this->request->post[$this->_moduleSysName . '_module'])) {
			foreach ($this->request->post[$this->_moduleSysName . '_module'] as $key => $value) {
				if (!$value['image_width'] || !$value['image_height'] || $value['image_height'] == '' || $value['image_width'] == '') {
