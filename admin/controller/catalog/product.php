...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->load->model('tool/neoseo_product_labels');
			$product_labels = isset($this->request->post['product_labels']) ? $this->request->post['product_labels'] : array();
			$this->model_tool_neoseo_product_labels->updateProductLabels($product_labels);
			if(isset($this->request->post['product_labels'])){
				unset($this->request->post['product_labels']);
			}
      ...
      if (isset($this->request->get['filter_category'])) {
        $url .= '&filter_category=' . $this->request->get['filter_category'];
			...
			if(isset($this->request->get['action']) && $this->request->get['action'] == 'save_and_stay' && isset($product_id) && $product_id > 0){
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->load->model('tool/neoseo_product_labels');
			$product_labels = isset($this->request->post['product_labels']) ? $this->request->post['product_labels'] : array();
			$this->model_tool_neoseo_product_labels->updateProductLabels($product_labels,$this->request->get['product_id']);
			if(isset($this->request->post['product_labels'])){
				unset($this->request->post['product_labels']);
			}
			//NeoSeo Product Labels - end
    ...
    if (isset($this->request->get['filter_category'])) {
      ...
      'filter_category' => $filter_category,
		...
		$data['categories'] = $this->model_catalog_category->getCategories($filter_data);
                ...
                $list_labels = $this->model_tool_neoseo_product_labels->getListLabels();

                $languages=$this->model_localisation_language->getLanguages();
                $system_languages=$languages[$this->config->get('config_language')]['language_id'];
                $data['product_labels'] = array();
                foreach ($list_labels as $label){
                        $array_name =  unserialize($label['name']);
                        $label['name']=$array_name[$system_languages];
                        $products = $this->model_tool_neoseo_product_labels->getLabelProducts($label["label_id"]);
                        if($products){
                                $label['products']=$products;
                        }
                        $data['product_labels'][] = $label;
                }
	...
	protected function getForm() {
		/* SOFORP Product Order History - begin */
		if (isset($this->request->get['product_id'])) {
			$product_id = $this->request->get['product_id'];
		}else{
			$product_id = 0;
		}
        $this->language->load('catalog/soforp_product_order_history');
		$this->load->model('catalog/soforp_product_order_history');
		$filter = array(
			'product_id' => $product_id,
		);
		$data['tab_orders'] = $this->language->get('tab_orders');
		$data['orders'] = $this->model_catalog_soforp_product_order_history->getOrders($filter);
		/* SOFORP Product Order History - end */
    //CKEditor
    if ($this->config->get('config_editor_default')) {
		...
		$data['entry_category'] = $this->language->get('entry_category');
                //NeoSeo Product Labels - begin -->
		$data['entry_product_labels'] = $this->language->get('entry_product_labels');
		//NeoSeo Product Labels - end
