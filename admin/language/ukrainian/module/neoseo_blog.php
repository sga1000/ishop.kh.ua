<?php
// Heading
$_['heading_title'] = '<img width="24" height="24" src="view/image/neoseo.png" style="float: left;"><p style="margin:0;line-height: 24px;">NeoSeo Блог</p>';
$_['heading_title_raw'] = 'Налаштування блога';

// Tab
$_['tab_general'] = 'Параметри';
$_['tab_logs'] = 'Логи';
$_['tab_fields'] = 'Поля';
$_['tab_support'] = 'Підтримка';
$_['tab_license'] = 'Ліцензія';

// Text
$_['text_success']                    = 'Налаштування змінені!';
$_['text_edit']                       = 'Змінити налаштування';
$_['text_support']                    = 'Підтримка модуля';
$_['text_module'] = 'Модулі';
$_['text_success_clear'] = 'Лог файл успішно очищений!';
$_['text_clear_log'] = 'Очистити лог';
$_['text_module_version'] = '';

// Button
$_['button_save'] = 'Зберегти';
$_['button_save_and_close'] = 'Зберегти і Закрити';
$_['button_close'] = 'Закрити';
$_['button_recheck'] = 'Перевірити ще раз';
$_['button_clear_log'] = 'Очистити лог';
$_['button_download_log'] = 'Завантажити файл логів';

// Entry
$_['entry_add_to_menu']               = 'Додати посилання на блог в меню:';
$_['entry_debug'] = 'Отладочный режим:';
$_['entry_comment_auto_approval']     = 'Автоперевірка коментарів:';
$_['entry_author_block_article']      = 'Додати інфомацию про автора до статті:';
$_['entry_product_limit']             = 'К-ть товарів в категорії:';
$_['entry_comment_limit']             = 'К-ть коментарів:';
$_['entry_cache_results']             = 'Кешувати результати:';
$_['entry_article_time_format']       = 'Формат часу для статті:';
$_['entry_comment_time_format']       = 'Формат часу для коментарів:';

$_['entry_meta_date']                 = 'Відображати дату в статті:';
$_['entry_meta_author']               = 'Відображати автора в статті:';
$_['entry_meta_comments']             = 'Відображати до коментарів в статті:';
$_['entry_meta_category']             = 'Відображати категорію в статті:';

$_['entry_heading']                   = 'Заголовок блогу:';
$_['entry_title']                     = 'Блог Title:';
$_['entry_meta_keyword']              = 'Meta Keywords:';
$_['entry_meta_description']          = 'Meta Description:';
$_['entry_articles_block_heading']    = 'Заголовок блоку статей:';
$_['entry_products_block_heading']    = 'Заголовок блоку товарів:';
$_['entry_comments_block_heading']    = 'Заголовок блоку коментарів:';
$_['entry_author_block_heading']      = 'Заголовок блоку автора:';
$_['entry_gallery_heading']           = 'Заголовок галереї:';
$_['entry_share_social_code']         = 'Код блоку "Поділитися":';

$_['entry_image_article']             = 'Розмір зображення статті';
$_['entry_image_article_list']        = 'Розмір зображення статті в категорії';
$_['entry_image_article_list_width']  = 'Ширина зображення статті в категорії';
$_['entry_image_article_list_height'] = 'Висота зображення статті в категорії';

$_['entry_image_article_block']       = 'Розмір зображення статті в блоку';
$_['entry_image_article_block_width']       = 'Ширина зображення статті в блоку';
$_['entry_image_article_block_height']       = 'Висота зображення статті в блоку';

$_['entry_image_product_block']       = 'Розмір зображення товару в блоці';
$_['entry_image_product_block_width']       = 'Ширина зображення товара в блоку';
$_['entry_image_product_block_height']       = 'Висота зображення товара в блоку';

$_['entry_image_category_block']      = 'Розмір зображення категорії';
$_['entry_image_category_block_width']      = 'Ширина зображення категорії';
$_['entry_image_category_block_height']      = 'Висота зображення категорії';

$_['entry_image_author_block']        = 'Розмір зображення автора';
$_['entry_image_author_block_width']        = 'Ширина зображення автора';
$_['entry_image_author_block_height']        = 'Висота зображення автора';

$_['entry_image_gallery_thumb']       = 'Розмір зображення галереї';
$_['entry_width'] = 'Ширина';
$_['entry_height'] = 'Высота';



// Help
$_['help_time_format'] = '<a href="http://php.net/manual/en/function.strftime.php" target="_blank">формати часу</a>';

// Error
$_['error_permission'] = 'У вас немає прав змінювати налаштування блогу!';
$_['error_download_logs'] = 'Файл логів порожній або відсутній!';
$_['error_ioncube_missing'] = '';
$_['error_license_missing'] = '';
$_['mail_support'] = '';
$_['module_licence'] = '';
$_['text_module_version'] = '';
