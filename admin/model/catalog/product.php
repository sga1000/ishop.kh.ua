...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
			...
			if (!empty($data['filter_manufacturer'])) {
				$query_check = $this->db->query("SHOW TABLES LIKE '%product_series'");
				if ($query_check->num_rows == 1) {
					$sql .= " LEFT JOIN " . DB_PREFIX . "product_series ps ON (p.product_id = ps.product_id)";
				}
			}
    ...
    if (!empty($data['filter_category'])) {
		...
		/* NeoSeo Action Manager - begin */
		if (!empty($data['filter_manufacturer'])) {
			$query_manufacturer = $this->db->query("SELECT * FROM " . DB_PREFIX . "manufacturer WHERE manufacturer_id = '" . (int)$data['filter_manufacturer'] . "'");
			if ($query_manufacturer->num_rows) {
				if (!isset($query_manufacturer->row['manuf_parent_id']) || $query_manufacturer->row['manuf_parent_id'] == 0) {
					$sql .= " AND p.manufacturer_id = '" . (int)$query_manufacturer->row['manufacturer_id'] . "'";
				} else {
					$sql .= " AND ps.series_id = '" . (int)$query_manufacturer->row['manufacturer_id'] . "'";
				}
			}
		}
		/* NeoSeo Action Manager - end */
